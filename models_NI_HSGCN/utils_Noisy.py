...
class FCLayer_snn(torch.nn.Module):

    def __init__(self, in_features, out_features, mu, sigma, bias=True, activation=None):
        super().__init__()

        self.linear = spiking_layer.Linear(in_features, out_features, bias=bias)
        if activation is None:
            self.activation = torch.nn.Identity()
        elif activation == 'NILIF':
            self.activation = NIIFNode(surrogate_function=surrogate.ATan(),mu=mu,sigma=sigma, detach_reset=True)
        else:
            raise NotImplementedError('activation layer [%s] is not found' % activation)
