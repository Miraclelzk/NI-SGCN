...
from .feature_Noisy import FeatureExtraction_snn_Noisy
from .score_snn import ScoreNet_snn
...
class DenoiseNet_snn_Noisy(nn.Module):
    ...
    def __init__(self, args):
        ...
        # networks
        self.feature_net = FeatureExtraction_snn_Noisy(self.T, mu=self.mu, sigma=self.sigma)
        self.score_net = ScoreNet_snn(
            self.T, 
    ...
    def get_supervised_loss(self, pcl_noisy, pcl_clean):
        ...
        pnt_idx = get_random_indices(N_noisy, self.num_train_points) # 随机一些点
        ...
        # snn
        if self.T != 1:
            pcl_noisy_T = pcl_noisy.unsqueeze(0).repeat(self.T, 1, 1, 1) #(T, B, N, 3)
        else:
            pcl_noisy_T = pcl_noisy

        # Feature extraction
        feat = self.feature_net(pcl_noisy_T)  # (T, B, N, F)

        if self.T != 1:
            feat = feat[:,:,pnt_idx,:]  # (T, B, n, F)  
            feat = feat.mean(0)
        else:
        ...
        # Denoising score matching
        # frames_centered(B, n, K, 3)  feat(B, n, F) -> grad_pred(B, n, K, 3)

        if self.T != 1:
            frames_centered = frames_centered.unsqueeze(0).repeat(self.T, 1, 1, 1, 1)  # (B, n, K, 3) -> (T, B, n, K, 3) 

            feat = feat.unsqueeze(0).repeat(self.T, 1, 1, 1)  # (B, n, F) -> (T, B, n, F)  

            grad_pred = self.score_net(
                x = frames_centered.view(self.T, -1, self.frame_knn, d),
                c = feat.view(self.T, -1, F),
            ).reshape(self.T, B, len(pnt_idx), self.frame_knn, d)   # (T, B, n, K, 3)
            grad_pred = grad_pred.mean(0)
        else:
            grad_pred = self.score_net(
    ...
    def denoise_langevin_dynamics(self, pcl_noisy, step_size, denoise_knn=4, step_decay=0.95, num_steps=30):
        ...
        if self.T != 1:
            pcl_noisy_T = pcl_noisy.unsqueeze(0).repeat(self.T, 1, 1, 1) #(T, B, N, 3)
            _ , B, N, d = pcl_noisy_T.size()
        else:
            pcl_noisy_T = pcl_noisy
            B, N, d = pcl_noisy_T.size()
        ...
        with torch.no_grad():
            ...
            for step in range(num_steps):
                ...
                if self.T != 1:
                    ...
                    grad_pred = self.score_net(
                        x = frames_centered.view(self.T, -1, denoise_knn, d),
                        c = feat.view(self.T, -1, F),
                    ).reshape(self.T, B, -1, d)   # (T, B, n*K, 3)
